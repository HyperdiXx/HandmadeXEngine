...
internal const Matrix4x4 IDENTITY_MATRIX = createMat4x4();
internal ColorRGB  default_text_color = createVec3(1.0f, 1.0f, 1.0f);
internal ColorRGB  default_cube_color = createVec3(0.0f, 1.0f, 0.0f);
internal ColorRGBA default_line_color = createVec4(1.0f, 0.0f, 0.0f, 1.0f);

internal uint32 default_line_width = 5;
internal real32 default_text_scale = 1.0f;

internal bool32 enable_shadows = false;
...
internal void setRenderPass(RenderPass *pass)
{
    active_render_pass = pass;
}

internal void setActiveFramebuffer(Framebuffer *fbo)
{
    active_framebuffer = fbo;
}

internal Framebuffer *getActiveFramebuffer()
{
    return active_framebuffer;
}

internal Shader *getShader(const char *name)
{
    return &shaders[name];
}

internal Texture2D *getTexture2DResource(const char *name)
{
    return &textures[name];
}

internal bool32 loadFont(const char *path)
{
    FT_Library lib;
...
internal bool32 loadFreeTextures()
{
...
internal bool32 initRenderDX11()
{
...
internal bool32 initCommonGpuResources()
{
...
internal void clear()
{
    ...
    std::unordered_map<const char*, Texture2D>::iterator it = textures.begin();
...
internal bool32 createLineMesh(glm::vec3 &start, glm::vec3 &end, LineMesh *line_com)
{
...
internal bool32 createLineMesh(LineMesh *line_com)
{
...
internal bool32 createLinesBuffer()
{
...
internal bool32 createQuadBuffer()
{
...
internal bool32 createRenderPass(const RenderPassData & data, RenderPass *rp)
{
...
internal bool32 createCubemap(std::vector<const char*> paths, Cubemap *cube)
{
...
internal bool32 createFullquad()
{
...
internal bool32 createSkybox(Skybox *sky)
{
...
internal bool32 createCubemap(Cubemap *cube)
{
...
internal bool32 createShadowMaps(ShadowMap *shadow)
{
...
internal bool32 createSphere(SphereMesh *sphre)
{
...
internal bool32 createCube(CubeMesh *cube)
{
    real32 data[] =
    {
...
internal void drawQuad(const glm::vec2 &pos, const glm::vec2 &size, const Color4RGBA &color)
{
    drawQuad({ pos.x, pos.y, 0.0f }, size, color);
}

internal void drawQuad(const glm::vec3 & pos, const glm::vec2 & size, const Color4RGBA& color)
{
...
internal void drawQuad(real32 x, real32 y, real32 w, real32 h, const Color4RGBA& color)
{
    drawQuad({ x, y, 0.0f }, { w, h }, color);
}

internal void drawQuad(real32 x, real32 t, real32 w, real32 h, Texture2D *texture)
{
    /*if (texture != nullptr)
    {
...
internal void drawQuad(Shader *shd, Texture2D *texture, Matrix4x4 &mod)
{
    /*if (texture != nullptr)
        graphics_device->activateBindTexture(TEXTURE_TYPE::COLOR, texture);
...
internal void drawModel(xe_assets::Model *mod, Shader *shd, const glm::mat4 &transform)
{
...
internal void drawText(const std::string &text, real32 x, real32 y, Vec3 &color, real32 scale)
{
...
internal void drawText(const std::string &text, Vec2 &pos, Vec3 &color)
{
    drawText(text, pos.x, pos.y, color, default_text_scale);
}

internal void drawText(const std::string &text, Vec2 &pos)
{
    glm::vec3 text_color = convertToVec3(default_text_color);
    drawText(text, pos.x, pos.y, text_color, default_text_scale);
}

internal void drawText(const std::string &text, real32 x, real32 y)
{
    glm::vec3 text_color = convertToVec3(default_text_color);
    drawText(text, x, y, text_color, default_text_scale);
}

internal void drawText(const std::string & text, real32 x, real32 y, real32 scale)
{
    glm::vec3 text_color = convertToVec3(default_text_color);
    drawText(text, x, y, text_color, scale);
}

internal void drawWaterPlane(Entity *ent)
{
...
internal void drawSkybox()
{
    ...
    Matrix4x4 view = glm::mat4(createMatrix3x3(camera.get_view_matrix()));
...
internal void drawAABB(const aabb &bb, const glm::mat4 &matrix)
{
...
internal void applyShadowMap(ShadowMap *shadow)
{
    real32 near_p = 1.0f;
    real32 far_p = 7.5f;
...
internal glm::mat4& applyTransform(xe_ecs::TransformComponent *transform, Shader *shd)
{
    glm::mat4 model_matrix = IDENTITY_MATRIX;
...
internal void applyDirLight(Shader *shd, xe_ecs::DirLight *directional_light, xe_ecs::TransformComponent *transform)
{
    GraphicsDevice *device = xe_render::getDevice();
...
internal void applySpotLight(Shader *shd, xe_ecs::SpotLight *spot_light, xe_ecs::TransformComponent * transform)
{


}

internal void applyPointLight(Shader *shd, xe_ecs::PointLight *directional_light, xe_ecs::TransformComponent *transform)
{

}

internal void beginFrame(bool32 shouldClearScreen)
{
...
internal void setupCommand(CommandType type)
{
    switch (type)
    {
...
internal
glm::vec3 convertToVec3(Color3RGB color)
{
    return glm::vec3(color.x, color.y, color.z);
}

internal
glm::vec4 convertToVec4(Color4RGBA color)
{
    return glm::vec4(color.x, color.y, color.z, color.a);
}

internal
void convertToVec4(Color3RGB color)
{
...
internal
void drawCube(Texture2D *texture_diff)
{
    if (texture_diff)
    {
...
internal
void drawLine(real32 x, real32 y, real32 z, real32 x_end, real32 y_end, real32 z_end, Color4RGBA color)
{
...
internal
void drawLine(real32 x, real32 y, real32 z, real32 x_end, real32 y_end, real32 z_end, Color3RGB color)
{
    drawLine(x, y, z, x_end, y_end, z_end, Color4RGBA(color.x, color.y, color.z, 1.0f));
}

internal
void drawLine(real32 x, real32 y, real32 x_end, real32 y_end, Color4RGBA color)
{
    if (graphics_state.line_index_count >= RenderState::max_line_ind)
    {
        //flushLines();
...
internal
void drawLine(real32 x, real32 y, real32 x_end, real32 y_end, Color3RGB color)
{
...
internal
void drawEntWithShader(xe_ecs::Entity *ent, Shader *shd)
{
    using namespace xe_ecs;
...
internal
void drawMesh(xe_assets::Mesh *msh, Shader *shd)
{
    GraphicsDevice *device = xe_render::getDevice();
...
internal
void drawSphere(Texture2D *texture_diff)
{
    if (texture_diff)
        graphics_device->activateBindTexture(TEXTURE_TYPE::COLOR, texture_diff);
    drawSphere();
