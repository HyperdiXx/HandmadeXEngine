...
{
    /*EditorState *app_state = application::getAppState();
    Scene &activeS = app_state->active_scene;
...
internal
void loadSpheresScene(Scene *scene_ptr)
{
    //@ PBR scene
...
inline Entity* getEntity()
{
    ...
    /*std::vector<Entity> &ents = gs.entities;
...
inline Entity *getEntityByType(ENTITY_TYPE type)
{
    Entity *result = nullptr;
...
void gameInit()
{
    ...
    /*Scene new_scene = createScene("TestScene");
    Scene pbr_scene = createScene("PBRScene");

    loadTestScene(&new_scene);
    loadSpheresScene(&pbr_scene);
